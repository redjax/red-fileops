...
class ScanDir:
    ...
    def __post_init__(self):
        assert self.path is not None, ValueError("@ScanDir: path cannot be None")
        assert isinstance(self.path, str) or isinstance(self.path, Path), TypeError(
            f"@ScanDir: path must be of type str or Path. Got type: {type(self.path)}"
        )
    ...
    def get_dirs_files(
        ...
        return ScanResults(files=files, dirs=dirs)
...
if __name__ == "__main__":
    ...
    SCAN_DIR.refresh_metadata(
        path_list=["./scan_results/results.json"]
    )
